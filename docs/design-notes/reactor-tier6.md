# Design Notes: Tier 6 Nuclear Reactor
...
## Interaction with Vertical Building

An elevated reactor converts its nuclear cell into steam **faster** (reduced phase duration), but does **not** produce more total steam per cell. The cell is consumed in fewer ticks, so the player needs more cells over time. This makes elevation a throughput trade-off, not a free power boost — useful for peak demand situations but more fuel-hungry.

See [vertical-building.md](vertical-building.md) for the full elevation system.
...
- Elevated reactor interaction: faster phase duration means more cells consumed per unit time — what's the exact multiplier? See [vertical-building.md](vertical-building.md)
